...
new_panel=open('members_panel.js',encoding='utf-8').read()
c=open('POLR-WIRED-ENHANCED.html',encoding='utf-8').read()
start=c.find('function MembersFullPanel(')
chunk=c[start:start+50000]
nl=chr(10)
offset=chunk.find(nl+'function ')
end=start+offset
print('Replacing',end-start,'chars with',len(new_panel),'chars')
c=c[:start]+new_panel+c[end:]
...
if r.returncode==0:
    ...
    subprocess.run(['git','commit','-m','Replace MembersFullPanel with Railway CRUD panel'])
